...
const vePhimReducer = createSlice({
  name: 'vePhimReducer',
  initialState,
  reducers: {
    datGhe: (state, action) => {
      let gheDaDat = action.payload;
      let index = state.arrGheDangDat.findIndex(ghe => {
        return ghe.soGhe === gheDaDat.soGhe
      })
    ...
    huyGhe: (state, action) => {
      ...
      state.arrGheDangDat = state.arrGheDangDat.filter((gheHuy) => {
        return gheHuy.soGhe !== huyGhe
      })
...
export const { datGhe, huyGhe} = vePhimReducer.actions

export default vePhimReducer.reducer